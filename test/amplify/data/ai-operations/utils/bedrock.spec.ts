...
describe('bedrock utilities', () => {
  ...
  describe('invokeBedrock', () => {
    it('should invoke Bedrock Nova with correct parameters', async () => {
      const mockResponse = {
    ...
    it('should use default max tokens and temperature for Nova', async () => {
      const mockResponse = {
        body: new TextEncoder().encode(
          JSON.stringify({
            output: {
      ...
      expect(bodyJson.inferenceConfig.max_new_tokens).toBe(4000);
      expect(bodyJson.inferenceConfig.temperature).toBe(0.3);
    });

    it('should accept custom max tokens for Nova', async () => {
      const mockResponse = {
        body: new TextEncoder().encode(
          JSON.stringify({
            output: {
      ...
      const commandCall = (InvokeModelCommand as unknown as Mock).mock.calls[0][0];
      const bodyJson = JSON.parse(commandCall.body);
      expect(bodyJson.inferenceConfig.max_new_tokens).toBe(2000);
    });

    it('should accept custom temperature for Nova', async () => {
      const mockResponse = {
        body: new TextEncoder().encode(
          JSON.stringify({
            output: {
      ...
      const commandCall = (InvokeModelCommand as unknown as Mock).mock.calls[0][0];
      const bodyJson = JSON.parse(commandCall.body);
      expect(bodyJson.inferenceConfig.temperature).toBe(0.5);
    });

    it('should format Nova request body correctly', async () => {
      const mockResponse = {
        body: new TextEncoder().encode(
          JSON.stringify({
            output: {
      ...
      const commandCall = (InvokeModelCommand as unknown as Mock).mock.calls[0][0];
      const bodyJson = JSON.parse(commandCall.body);
      expect(bodyJson).toHaveProperty('schemaVersion', 'messages-v1');
      expect(bodyJson.system[0]).toEqual({ text: 'sys' });
      expect(bodyJson.messages[0]).toEqual({
        role: 'user',
        content: [{ text: 'usr' }],
      });
      expect(bodyJson.inferenceConfig).toHaveProperty('max_new_tokens');
      expect(bodyJson.inferenceConfig).toHaveProperty('temperature');
    });
  ...
  describe('retryWithSchema', () => {
    ...
    it('should use retry temperature (0.1)', async () => {
      ...
      const commandCall = (InvokeModelCommand as unknown as Mock).mock.calls[0][0];
      const bodyJson = JSON.parse(commandCall.body);
      expect(bodyJson.inferenceConfig.temperature).toBe(0.1);
    });
  ...
  describe('invokeAiWithRetry', () => {
    ...
    it('should throw if retry also fails to parse', async () => {
      ...
      mockSend.mockResolvedValue({
        body: new TextEncoder().encode(
          JSON.stringify({
            output: { message: { content: [{ text: 'still invalid' }] } },
          })
        ),
      });
    ...
    it('should handle complex nested types', async () => {
      ...
      const mockResponse = {
        body: new TextEncoder().encode(
          JSON.stringify({
            output: { message: { content: [{ text: JSON.stringify(complexData) }] } },
          })
        ),
      };
      mockSend.mockResolvedValue(mockResponse);
