...
<template>
  <UCard data-testid="stalled-preview-section">
    <template #header>
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div class="space-y-1">
          <h2 class="text-lg font-semibold">{{ t('dashboard.pipeline.stalled.title') }}</h2>
          <p class="text-sm text-dimmed">{{ t('dashboard.pipeline.stalled.description') }}</p>
        </div>
        <UButton
          :label="t('dashboard.pipeline.stalled.cta.openPipeline')"
          to="/pipeline"
          color="neutral"
          variant="outline"
          size="sm"
          data-testid="stalled-preview-open-pipeline-link"
        />
      </div>
    </template>
